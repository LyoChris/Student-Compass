...
export default function RegisterPage() {
  ...
  // Reload faculties whenever city changes
  useEffect(() => {
    ...
    <div className="min-h-screen bg-slate-900 flex overflow-hidden">
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden items-center justify-center p-12">
        <div
          ...
          style={{
            background:
              'linear-gradient(135deg, rgba(99,102,241,0.12) 0%, transparent 60%, rgba(168,85,247,0.08) 100%)',
          }}
        />
        ...
        <div className="relative z-10 text-center max-w-sm">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            {FEATURE_ICONS.map(({ Icon, color }, i) => (
              <div
                key={i}
                className="w-12 h-12 rounded-2xl glass-card flex items-center justify-center hover:-translate-y-1"
              >
          ...
          <div className="mt-8 glass-card rounded-2xl p-4 text-left">
            {[
              ...
              'Bank-grade security (JWT + HttpOnly cookies)',
      ...
      <div className="flex-1 flex items-start justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-sm py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <InputField label="City">
              <div className="relative">
                <select
                  ...
                  className={`${SELECT_CLS} ${form.cityId ? 'text-slate-100' : 'text-slate-600'}`}
                >
                  <option value="">
            ...
            {/* Faculty — enabled only after city selected */}
            <InputField label="Faculty">
              <div className="relative">
                <select
                  ...
                  onChange={handleChange}
                  disabled={!form.cityId || loadingFaculties}
                  className={`${SELECT_CLS} ${form.facultyId ? 'text-slate-100' : 'text-slate-600'}`}
                >
                  <option value="">
                    {loadingFaculties
                      ? 'Loading faculties…'
                  ...
                  {faculties.map((f) => (
                    <option key={f.id} value={f.id} className="bg-slate-800 text-slate-100">
                      {f.name}
                    </option>
                  ))}
                </select>
